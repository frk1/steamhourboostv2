...
module.exports = class SteamAccount extends EventEmitter {
  constructor(name, password, sentry, secret, games, indent = 0, customtitle) {
    super()
    this.name = name
    ...
    if(this.customtitle) {
      this.games.unshift(customtitle)
    }
  ...
  boost() {
    return this.login()
      .then(() => {
        this.client.setPersona(this.customtitle ? SteamUser.EPersonaState.Online : SteamUser.EPersonaState.Offline)
